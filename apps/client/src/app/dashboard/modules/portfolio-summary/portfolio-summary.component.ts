...
/**
 * Portfolio Summary dashboard module wrapper.
 *
 * Container/isolation wrapper for the dashboard grid canvas. Unlike the other
 * (self-fetching) module wrappers, the embedded `GfPortfolioSummaryComponent`
 * (`<gf-portfolio-summary />`) is purely presentational — it exposes nine
 * `@Input()`s and a single `emergencyFundChanged` `@Output()`. This wrapper
 * therefore acts as the container, sourcing every input from the existing
 * application services (`DataService`, `UserService`,
 * `ImpersonationStorageService`, `DeviceDetectorService`) and handling the
 * emergency-fund output — replicating the established container logic of
 * `GfHomeSummaryComponent`.
 *
 * Module-isolation contract (AAP §0.8.1): this wrapper never imports or
 * references the grid canvas, the module registry, the layout service, or the
 * shared dashboard layout types. It holds feature DATA state only; all grid /
 * layout geometry remains owned by the canvas. The optional `removeModule`
 * callback input is the sole channel through which the canvas detaches the
 * module — no `@Output()` is used for removal.
 */
@Component({
