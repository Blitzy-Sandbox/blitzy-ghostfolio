...
/**
 * GfWatchlistModuleComponent
 *
 * Thin, standalone "isolation wrapper" that re-hosts the existing,
 * self-fetching {@link GfHomeWatchlistComponent} feature as an independently
 * placeable dashboard grid module. It supplies the grid chrome — an outlined
 * MatCard surface, a draggable header carrying the localized "Watchlist"
 * title, and a remove (close) icon-button — while delegating all data
 * concerns to the wrapped feature component.
 *
 ...
 * `<gf-home-watchlist />` with no input/output bindings.
 *
  ...
  template: `
    <mat-card appearance="outlined" class="gf-module-card">
      <div class="gf-module-header gf-module-drag-handle">
        <span class="gf-module-title" i18n>Watchlist</span>
        <button
          aria-label="Remove Watchlist module"
          i18n-aria-label
          i18n-matTooltip
          mat-icon-button
