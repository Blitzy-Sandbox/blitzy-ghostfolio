...
describe('UserDashboardLayoutService', () => {
  ...
  beforeEach(() => {
    // The service is single-arg by design (Prisma only). Prometheus metrics
    // for the layout endpoints are owned solely by
    // `UserDashboardLayoutObservabilityMiddleware` (covered by
    // `user-dashboard-layout.middleware.spec.ts`), so this service spec asserts
    // only the Rule 8 `where: { userId }` scoping and the structured error
    // logging — never metric emission.
