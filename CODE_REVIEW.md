...
| --------------------------------------- | ----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| New client grid-system source files     | _TBD_ | `dashboard-canvas/`, `module-registry.service.ts`, `module-catalog/`, `dashboard-layout.service.ts`, `modules/**` wrappers, `dashboard.types.ts` under `apps/client/src/app/dashboard/**`                                    |
| New API layout-feature source files     | _TBD_ | `user-dashboard-layout.controller.ts`, `user-dashboard-layout.service.ts`, `dtos/update-user-dashboard-layout.dto.ts` under `apps/api/src/app/user/**`                                                                       |
| New shared-library files                | _TBD_ | `libs/common/src/lib/interfaces/user-dashboard-layout.interface.ts`                                                                                                                                                          |
| New Prisma model + migration            | _TBD_ | `UserDashboardLayout` model (1:1 → `User`, cascade delete) in `prisma/schema.prisma`; additive `CREATE TABLE` migration under `prisma/migrations/**`                                                                         |
| New API endpoints                       | _TBD_ | `GET /api/v1/user/layout`, `PATCH /api/v1/user/layout`                                                                                                                                                                       |
...
| Navigation-shell edits (existing files) | _TBD_ | `app.routes.ts`, `app.component.{ts,html,scss}`, `components/header/header.component.{ts,html,scss}`, `pages/portfolio/portfolio-page.html`, `pages/portfolio/portfolio-page.component.ts`, `ngsw-config.json` (conditional) |
| Wiring-only edits (existing files)      | _TBD_ | `apps/api/src/app/user/user.module.ts`, `libs/common/src/lib/interfaces/index.ts`, `package.json`                                                                                                                            |
| Governance / observability deliverables | _TBD_ | `CODE_REVIEW.md` (this file), `blitzy-deck/*dashboard*exec*summary*.html`, decision log + bidirectional traceability matrix (Markdown), observability dashboard template / runbook                                           |
| **Total in-scope files**                | _TBD_ | Confirmed by Phase 8 (Principal Reviewer) after all domain phases resolve                                                                                                                                                    |
